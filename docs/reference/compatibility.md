...
| Input `background_tasks` on `Stop`: entries `{type: "subagent", status: "running", …}` while a background subagent runs (verified 2.1.282 headless, 2026-09-25, `claude -p` with a `--settings` Stop hook logging its input: listed at the stop that launched it; `claude -p` stays open and fires `Stop` again when it finishes). Not yet verified: that the bundled `/code-review` fork is listed as `subagent` in an interactive session (`-p` runs it in the foreground) | `loop-dev-gate` — a stop on unchanged code while a subagent runs is a free wait, not a review round. Field missing → every stop charges, as before. Limits: the field cannot distinguish a grader from any other subagent, so a decoy subagent buys up to 8 allowed stops per round — pauses with the loop still armed, never a stamp; a headless `claude -p` stays open until a hung subagent ends, so wrap CI runs in a timeout; and `.cc-loop-dev-rounds` is agent-writable, the same trust class as the rounds count always was. The round fingerprint is the working tree's content (tracked + untracked, `.cc-*` and untracked embedded repos such as grader worktrees excluded, assume-unchanged bits cleared), but it follows the repo's ignore rules, so a new file hidden via `.git/info/exclude` or `core.excludesFile` is invisible to it: dropping those would mean dropping `.gitignore` too. Like the decoy case, that buys free waits, never a stamp |
