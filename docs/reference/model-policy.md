...
| `security` and `security-review` graders | Session model — **never downgrade** | `loop-dev.md` step 5 |
...
| Review sub-agent (`design-reviewer`) | Session model — **unpinned by design** | no `model:` frontmatter |
...
### Fan-out cost (measured 2026-09-10)
...
These figures are a **snapshot** (2026-09-10, ~11,200 turns of transcript). The corpus grows with every session, so re-running gives different absolute numbers — turn counts climb, and shares and amplification drift by a few tenths. That is expected and is not a discrepancy. **The finding is the ratios and the order of magnitude**, which have held across every run: a subagent turn is roughly an order of magnitude cheaper than a main-thread turn, subagents are a low-single-digit share of spend, and a perfect read-shunt addresses well under one percent.

| Where the work ran | Share of cost | Turns | $/turn | Median context/turn |
|---|---|---|---|---|
| Interactive session (main thread) | 94.9% | 8,050 | $0.352 | 379k |
| Subagent | 5.1% | 3,193 | $0.048 | 62k |

**What this shows, and what it does not.** A subagent turn costs ~7× less than a main-thread turn because it works in a 62k context instead of a 379k one. That is close to definitionally true, and it is **not** a refutation of Systima's "The Subagent Tax" ([systima.ai/blog/subagent-tax](https://systima.ai/blog/subagent-tax), ~2026-07), which measured *the same work* done sequentially versus fanned out at 2.6×–5.9× the tokens. This sample contains no sequential counterfactual, and per-locus accounting cannot produce one: a subagent's report lands in the parent's context and is re-read on every later parent turn, and that cost is charged to the parent, never to the subagent.

What the sample does support is narrower and still decides the question: **subagents were 5.1% of all spend.** No grader panel in this repo is a material cost line. The scaling rules in `loop-dev.md` step 5 stay as they are, but their justification is latency and review noise, not cost — **never skip a grader to save tokens.**
...
|---|---|---|---|---|
| largest | 3,832 | 47k | 615k | 13.2× |
| | 1,263 | 79k | 125k | 1.6× |
| | 1,014 | 60k | 747k | 12.5× |
| | 572 | 75k | 698k | 9.3× |
| | 380 | 71k | 315k | 4.4× |

Median growth excluding the largest session is **4.2×**, so this is not an artifact of one runaway session. The effect is within a session, turn over turn, not across sessions. (An earlier revision also claimed the largest session was 57% of spend against 53% of turns, and so proportional to its length; the script prints no per-session spend, so that comparison is not re-derivable and has been dropped rather than carried forward unverified. It is 48% of main-thread turns.)
...
| | |
|---|---|
| File-read text reaching context | **1.14M tokens** — 808k via Bash (`cat`/`head`/`sed -n`/`tail`), 336k via the `Read` tool |
| Share of all tool-result text | 64% |
| Average re-read amplification (`cache_read` ÷ `cache_creation`) | 21.6× |
| Amplified file-read cost | 24.67M of 3,394M `cache_read` tokens |
| **Share of spend a perfect shunt could address** | **0.73%** |
...
**Why delegation still pays when the shunt does not.** They are different mechanisms and the numbers are not in conflict. A shunt removes tool-result *bytes* from the parent, worth ~0.7%. Delegation removes *turns* from the parent: exploration that takes fifteen turns costs fifteen turns at the parent's context size, versus fifteen at a subagent's 62k with only the conclusion coming back. The saving is in the turn count, not the payload.

**Fork dispatch for graders.** v2.1.232 made forked subagents available without the `CLAUDE_CODE_FORK_SUBAGENT=1` env var that had gated them since v2.1.117. It did **not** make every subagent a fork: `subagent_type: "fork"` forks the caller, while any other type — or omitting it — starts a fresh agent. A grader dispatch that names no type is therefore already a fresh context, which is what the 62k above measures. Keep it that way. A fork would swap that 62k for the parent's context, always runs on the parent's model so the tiering above cannot apply to it, and inherits the author's reasoning, which is the opposite of an independent review.

## Pinning a model

- **Agents**: `model: sonnet | opus | haiku` in the agent frontmatter. **No agent in this repo pins a model** — `architect:design-reviewer` dropped its `sonnet` pin in `architect` 2.0.3, because a pinned alias is resolved before dispatch and fails against a non-Anthropic endpoint. `security:finding-verifier` is unpinned for a different reason: its job is to disprove a Critical/High security finding, and a cheaper model that either rubber-stamps or over-refutes is worse than running no verification at all — an over-eager refutation deletes a real vulnerability from the report. This is the same reasoning that keeps the `security` grader on the session model.
