...
### Fixed
- **`harness` `2.3.5`** — `design-check.sh` recognised a code fence closer only at column 0, while CommonMark lets it be indented up to 3 spaces. An indented closer ended the block in the rendered design but not in the gate, so every decision after it — open ones included — was invisible until the next fence, and the lock passed (fail-open). The gate now reads only the fence lines every reader agrees on and blocks on the rest rather than guess, because a guess either way let an open decision through: a closer is 0–3 spaces, a run of the opener's character at least as long, then only spaces (or a CRLF `\r`); an opener stays at column 0. An indented opener (a fence at top level, but one that ends with its list item inside a list) and a near-miss closer (the run followed by a tab, the other fence character or other whitespace, which CommonMark and the map's renderer read differently) now block with the line number and how to fix it. A shorter run, a line with an info string, or 4+ spaces of indent stay plain content. Bytes the gate and the renderers split into lines differently also block before parsing: a carriage return not followed by a newline, a NUL byte, or invalid UTF-8. `render-map.sh` used the same column-0 rule, so the page a reviewer reads could disagree with the gate; it now applies the same rules, and a design that passes the gate contains no fence line the two read differently. (XARI-151)
