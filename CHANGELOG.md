...
### Fixed
- **`harness` `2.3.6`** — `design-check.sh` read a design line by line, so Markdown the renderers parse differently could still show a reviewer an open decision the gate never checked: an HTML comment around a fence opener, a `## Decisions` heading inside a list or blockquote, one spelled `## *Decisions*`, `## Decision&#115;` or `<h2>Decisions</h2>`, and inside the record an escaped marker (`\- [ ] Q2`), a table row or a lazy continuation all passed. Blocking constructs one by one did not converge, and neither did modelling block structure — three review rounds each found a paragraph, list or quote shape the gate read differently from CommonMark or marked. So the gate now accepts only a dialect it can judge **one line at a time**, and anything outside it blocks. **Checkboxes live only in the record:** outside `## Decisions` any `[ ]`, `[x]`, `[X]` or `[~]` blocks, whatever the list marker, nesting or heading above it, read as a reader sees it (through escapes, code and emphasis markers, with entities outside the record limited to `&lt; &gt; &amp; &quot; &#39;`), so to-dos elsewhere are written as plain bullets — recognising which headings a reader takes as "the record" (translations, look-alikes, "Decision log", sub-sections) never converged. **The record:** every non-blank line under `## Decisions` is a decision line, with no raw HTML, `&entity;`, link, `~` after its marker (marked strikes through single tildes), code fence or second checkbox in its text, any of which can show a reader something other than what the gate reads. **Headings** are ATX at column 0 and plain text — no heading in a list or indented, and no inline markup (backticks, `*`, `_` outside a word, `~`, `\`, `[`, `<`, entities) — and a second heading whose ASCII letters are exactly "Decision" or "Decisions" blocks (`### Decisions:`, `## Decision`, `## Decisions ##`), judged on letters alone so no invisible character splits the word. **No setext headings:** a line of only `-`, `=`, `*` or `_` must follow a blank line, a heading or a fence, so `---` directly under text now blocks (add a blank line above it). **No raw HTML anywhere**, code spans included — `<` then a letter, `/`, `!` or `?` — except an autolink or a mid-line `<br>` (a line that opens with one starts an HTML block that swallows what follows), because the map page keeps tags like `<h2>`, `<ul>` and `<input>`; write `&lt;` or use a fence. **No blockquotes, no link reference definitions** (any `]:` outside a `[[wikilink]]`), and **no tab, non-ASCII space or common invisible format character** (zero-width space, soft hyphen, every bidi control) outside fences, since renderers read them as indentation or heading separators. **Frontmatter** holds only the keys `/harness:shape` writes (`slug`, `status`, `stage`, `discovery`, `discovery-status`) and `- value` items holding one path or `[[wikilink]]` (quoted values may contain spaces), since a plain renderer shows it as body text. The shipped template and designs shaped like `/harness:shape` output pass unchanged. Look-alike headings (other languages, `Decisi0ns`, a Cyrillic "е") can still exist, but with checkboxes confined to the record none can hold an open item. Not checked: Unicode box shapes (☐) and text inside Mermaid diagrams. `render-map.sh` now skips CRLF frontmatter as the gate does, so a heading written inside it no longer becomes a tab. `/harness:shape`'s lock stage sends dialect blocks to a fix in place. (XARI-160)

## [marketplace 6.3.5] — 2026-09-25

### Fixed
- **`harness` `2.3.5`** — `design-check.sh` recognised a code fence closer only at column 0, while CommonMark lets it be indented up to 3 spaces. An indented closer ended the block in the rendered design but not in the gate, so every decision after it — open ones included — was invisible until the next fence, and the lock passed (fail-open). For fences, the gate now reads only the lines every reader agrees on and blocks on the rest rather than guess, because a guess either way let an open decision through: a closer is 0–3 spaces, a run of the opener's character at least as long, then only spaces; an opener is at column 0. **Any code block indented under a list item or paragraph now blocks** (once, with its line number: move it out of the list, to column 0), because inside a list it ends with the item and the gate cannot tell. A near-miss closer — the run followed by a tab, the other fence character or other whitespace, which CommonMark and the map's renderer read differently — blocks too. An indented run with a backtick in its info string is inline code and is left alone. Bytes the gate and the renderers split into lines differently block before parsing: a carriage return not followed by a newline, a NUL byte, or invalid UTF-8. A heading shown as "Decisions" in any form other than `## Decisions` at column 0 (indented, extra spaces or a tab, another level, another case, setext) blocks — it was a second record the gate never read. Valid CRLF designs now pass, and author text echoed in a block message has control bytes replaced. `render-map.sh` used the same column-0 fence rule, so the page a reviewer reads could disagree with the gate; it now applies the same rules, and the two agree on every fence line the gate accepts. Raw HTML blocks and "Decisions" headings nested in a list or blockquote or spelled with markup or entities were left open here and are closed in `2.3.6`. (XARI-151)
...
### Changed
- **`security` `2.0.2`** — `code-audit` 616 → **433 words**. Steps 1-5 were an OWASP checklist, which is precisely what `first-party-overlap.md` calls commodity: *"if deleting a skill would cost a gate, keep it; if it would only cost a checklist, delegate it."* The checklist moves to `references/owasp-checklist.md` and the body keeps the falsification pass — the gate `/harness:loop-dev`'s `security` grader depends on, and the only reason this skill is worth owning over a first-party one. Nineteen rules are now asserted, including the subtle ones: the verifier gets the claim, severity and `file:line` but **not** the reasoning, because a verifier shown the argument tends to agree with it; and a refuted finding moves to **Refuted** rather than being deleted, because a verifier can be wrong.
