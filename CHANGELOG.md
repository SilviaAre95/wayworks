...
### Fixed
- **`harness` `2.3.6`** — `design-check.sh` read a design line by line, so Markdown the renderers parse differently could still show a reviewer an open decision the gate never checked: an HTML comment around a fence opener, a `## Decisions` heading inside a list or blockquote, one spelled `## *Decisions*`, `## Decision&#115;` or `<h2>Decisions</h2>`, and inside the record an escaped marker (`\- [ ] Q2`), a table row or a lazy continuation all passed. Blocking constructs one by one did not converge, and neither did modelling block structure — three review rounds each found a paragraph, list or quote shape the gate read differently from CommonMark or marked. So the gate now accepts only a dialect it can judge **one line at a time**, and anything outside it blocks. **The record:** every non-blank line under `## Decisions` is a decision line, with no raw HTML, `&entity;` or code fence. **Headings** are ATX at column 0 and plain text — no heading in a list or indented, and no inline markup (backticks, `*`, `_` outside a word, `~`, `\`, `[`, `<`, entities) — and only `## Decisions` may read as the record: a heading whose ASCII letters are exactly "Decision" or "Decisions" blocks (`### Decisions:`, `## Decision`, `## Decisions ##`), and under any heading whose letters start with "Decision" (`## 1. Decision log`, `### Decisions—open`) a checkbox line blocks until the next heading. Letters alone, so no invisible or control character can split the word, while a title like `# Decision support tool` or `## Decisiones pendientes` passes. A heading with no letter or digit (`## `) blocks: it ended the record for the gate while the page showed nothing there. **No setext headings:** a line of only `-`, `=`, `*` or `_` must follow a blank line, a heading or a fence, so `---` directly under text now blocks (add a blank line above it). **No raw HTML anywhere**, code spans included — `<` then a letter, `/`, `!` or `?` — except an autolink or a mid-line `<br>` (a line that opens with one starts an HTML block that swallows what follows), because the map page keeps tags like `<h2>`, `<ul>` and `<input>`; write `&lt;` or use a fence. **No blockquotes, no link reference definitions** (any `]:` outside a `[[wikilink]]`), and **no tab, non-ASCII space or common invisible format character** (zero-width space, soft hyphen, bidi marks) outside fences, since renderers read them as indentation or heading separators. **Frontmatter** holds only the keys `/harness:shape` writes (`slug`, `status`, `stage`, `discovery`, `discovery-status`) and `- value` items holding one path or `[[wikilink]]` (quoted values may contain spaces), since a plain renderer shows it as body text. The shipped template and designs shaped like `/harness:shape` output pass unchanged. Not covered: Unicode lookalikes in a heading (a Cyrillic "е" in "Decisions"), ASCII look-alike spellings (`Decisi0ns`), headings that mention decisions later in their text, and bold or table look-alikes of the heading. `render-map.sh` now skips CRLF frontmatter as the gate does, so a heading written inside it no longer becomes a tab. `/harness:shape`'s lock stage sends dialect blocks to a fix in place. (XARI-160)
