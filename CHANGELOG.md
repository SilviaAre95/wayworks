...
### Fixed
- **`harness` `2.3.6`** — `design-check.sh` read a design line by line, so Markdown the renderers parse differently could still show a reviewer an open decision the gate never checked: an HTML comment around a fence opener, a `## Decisions` heading inside a list or blockquote, one spelled `## *Decisions*`, `## Decision&#115;` or `<h2>Decisions</h2>`, and inside the record an escaped marker (`\- [ ] Q2`), a table row or a lazy continuation all passed. Each fix since XARI-151 found another such construct, so the gate now accepts only a dialect it reads the same way CommonMark and marked do, instead of blocking constructs one by one. **Every non-blank line under `## Decisions` must be a decision line** (fences aside), and a decision line may hold no raw HTML (not even `<br>`) and no `&entity;`. **Raw HTML blocks anywhere, code spans included** — `<` then a letter, `/`, `!` or `?`, inline or not — except an autolink or `<br>`, because the map page keeps tags like `<h2>`, `<ul>` and `<input>`, and renderers pair backticks across escapes, lines and table cells in ways a line rule cannot follow; write `&lt;` or use a code fence. **Link reference definitions** (`[x]: url`) block too — marked lets their titles span lines — so links are written inline. **Frontmatter holds only the keys `/harness:shape` writes** (`slug`, `status`, `stage`, `discovery`, `discovery-status`) and indented `- value` items, since a plain renderer shows it as body text. **Headings must be top-level plain text:** a heading inside a list, a blockquote or an indented block blocks, and so does one using inline markup (backticks, `*`, `_` outside a word, `\`, `[`, `<`, or an entity), so a title like ``# Offline `stamp` queue`` now needs rewording. Only `## Decisions` may start with "Decision" — `### Decisions:`, `## Decision` or `## Decisions ##` block. Structure is read with tabs expanded to 4-column stops, so a tab after `>` or a list marker is a container. A setext heading's text is the whole paragraph its underline closes — a lone `-` under a paragraph is an underline, and a rule or underline indented past the paragraph is text — so a multi-line paragraph ending in "Decisions" above `---` passes, and a list item or lazy continuation above a less-indented `---` is a rule, not a heading. The shipped template and designs shaped like `/harness:shape` output pass unchanged. Not covered: Unicode lookalikes in a heading (a Cyrillic "е" in "Decisions"), which no text rule can tell from what a reader sees. `render-map.sh` now skips CRLF frontmatter as the gate does, so a heading written inside it no longer becomes a tab. `/harness:shape`'s lock stage sends dialect blocks to a fix in place. (XARI-160)
