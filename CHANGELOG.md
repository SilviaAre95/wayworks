...
### Changed
- **`shared` `2.2.6`** — `linear-update` had grown to 712 words, well past the 300-450 house range, because three rounds of fixes each added a rule and none removed prose. Trimmed to **446**. Every rule survives: all seven event rows and the state each maps to, the 80-word comment cap, the marker-**and**-URL duplicate check with its required `_(linear-update: <event>)_` marker line, the `links` attachment step, and all five Constraints bullets.

  What the range cost, itemized: the worked `stood-down` example; the lead paragraph; the standalone paragraph after the output-format block, which said the marker line is the only signal that makes a re-run idempotent without hiding a *different* event on the same PR; the rationale behind steps 2 and 5 (including step 5's note that re-attaching the same URL is idempotent); and the explanatory second sentence of the first Constraints bullet. That list is complete — every sentence in `main` was matched against the trimmed file, and the only other differences are rewordings that preserve meaning. The rules survive; several of the reasons they exist do not — and that reasoning is what argued a future editor out of "simplifying" the two-part duplicate check back into the bug 6.0.0 had just fixed. **Nothing automated replaces it.** `lint-skills.sh` has no word-count or content check, so `make check` is green at 446 or at 712, and the range is an AGENTS.md convention rather than a gate.

  Three places where compression had quietly become a *behavior* change were caught by the review panel and reverted: step 4's state lookup had turned unconditional (it is again conditional on the workspace's names differing, sparing an MCP round-trip on every event), the `In Review` constraint had lost "in the comment" as the place the PR URL must appear, and the `stood-down` row had lost its "in one clause" brevity bound.

  Why prose had to go rather than merely tighten: frontmatter (57 words), the seven-row event table (113, of which 32 are the bare `|` tokens `wc -w` counts as words), and the output-format block (35) are **205 words of required scaffolding** before a single step or constraint is written, leaving 241 for six steps and five bullets inside a 450 ceiling.
