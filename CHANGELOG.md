...
### Fixed
- **`harness` `2.3.6`** — `design-check.sh` read a design line by line, so Markdown the renderers parse differently could still show a reviewer an open decision the gate never checked: an HTML comment around a fence opener, a `## Decisions` heading inside a list or blockquote, one spelled `## *Decisions*`, `## Decision&#115;` or `<h2>Decisions</h2>`, and inside the record an escaped marker (`\- [ ] Q2`), a table row or a lazy continuation all passed. Each fix since XARI-151 found another such construct, so the gate now accepts only a dialect it reads the same way CommonMark and marked do, instead of blocking constructs one by one. **Every non-blank line under `## Decisions` must be a decision line** (fences aside), so notes, sub-headings and wrapped lines there now block. **A raw HTML block line blocks anywhere** — a line opening, after any quote or list markers, with `<` and a letter, `/`, `!` or `?` — with autolinks exempt; `<details>` or `<!-- … -->` in a design must move into a code fence. **Headings must be top-level plain text:** a heading inside a list or blockquote blocks, and so does one using inline markup (backticks, `*`, `_` outside a word, `\`, `[`, `<`, or an entity), so a title like ``# Offline `stamp` queue`` now needs rewording. A setext heading's text is now the whole paragraph its underline closes, so a multi-line paragraph ending in "Decisions" above `---` passes, and a list item or lazy continuation above a less-indented `---` is a rule, not a heading. The shipped template and designs shaped like `/harness:shape` output pass unchanged. Not covered: Unicode lookalikes in a heading (a Cyrillic "е" in "Decisions"), which no text rule can tell from what a reader sees. `/harness:shape`'s lock stage sends these blocks to a fix in place; `render-map.sh` still renders such a draft as-is. (XARI-160)
